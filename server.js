...
function normalizeChatMessages(messages, system) {
  ...
  const sys = system || (embeddedSystem && embeddedSystem.content) ||
    'Ты ChatClaud. Отвечай на языке пользователя. Не упоминай внутренние API, ключи и служебные детали.';
  ...
  if (pathname === '/api/chat' && req.method === 'POST') {
    try {
      ...
      try {
        ...
        const looksLikeQuery = forceSearch || (q3.length >= 8 && !greetingRe.test(q3));
